...
Gulia et al. \cite{gulia2019hybrid} mix ant colony optimization, which is a swarm intelligence algorithm,  and EAs to select
...
>>>>>>> dcb9dddd4262278fbcc6e1cae0dd7d5b78bfe9bf
...
<<<<<<< HEAD
The results obtained by the experiment show (see Figure \ref{fig:coco:1}) how
...
populations. We can see that the GA algorithm has a hard time reaching even the
...
dimensions. On the other hand, The multi-population PSO has better performance,
...
on $f_3$ and $f_4$. These results are also competitive when compared against
other GA and PSO implementations of past BBOB workshops. In particular, the
simple binary GA algorithm of Nicolau \cite{nicolau2009application}, the PSO
algorithm by El-Abd and Kamel \cite{el2009black} and an EDA and PSO hybrid
\cite{el2009blackHybrid}.

=======
The results (see Figure \ref{fig:avg}) obtained show  that the proposed algorithm reaches the
...
% This will have to be expanded a tiny bit - JJ
% expanded a bit - Mario
