...
individuals of the merged population and generates a new population
containing the best half from each. Finally, the 
method pushes the three new generated populations to the \texttt{messages Observable}. From
...
the runtime scales with dimension to reach certain target values $\Delta f$; Figure \ref{fig:avg} shows that the number of evaluations of {\sf GA-PSO-m} is competitive with PSO in the same framework, although the success rate is higher (Figure \ref{fig:bbob}). % Please check that this is true - JJ
