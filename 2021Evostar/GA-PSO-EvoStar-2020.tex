...
% --- I have moved this up from SoA. But we need to integrate it better
\begin{table}[h!tb]
...
This fact, % two paragraphs have been introduced, so we need to change
           % this - JJ
together with the a priori ease of simultaneous evaluation
...
% We shouldn't use the term "algorithms", since it's mixed-algorithm. We should
% use methods. 
...
compensates % compensates, in which sense? - JJ
for
...
search strategies have. % We should make a stronger statement on why
...
\cite{guervos2018introducing}) and a reactive migration procedure. % Also first time, we should make a statement with motivation for this procedure- JJ

We compare three heterogeneous configurations using a randomized parameter
technique. % Why do we do this? - JJ
...
scalability but also in the search performance. % Objectives should go
                                % first, not last - JJ
...
% I miss a sentence here explaining why we're talking about
% this. Maybe referring back to something in the introduction - JJ
...
mixing different algorithms might be
...
This is why methods that mix several population-based algorithms have been repeatedly
...
algorithms. % But we need to justify that the reason why they've have
            % done it is the same we have mentioned before - JJ

% a paragraph on the similarity of PSO and GA was moved up, taking out
% the meaning of "this". We need to revise that. Probably swap the
% previous paragraph and this one - JJ
