...
$\Delta f$. Values of targets $\Delta f = 10^{k}$ with $k$ colors are
given in the legend of the first function, $k$ values are $[-8,-5,-3,-2,-1,0,1]$.  
Line colors indicate the average of FEs for targets reached
at least one time. A red circle means that the algorithm
has reached the most challenging target $10^{-8}$ on all instances for that dimension,
if it has a number, this tell us how many times it was reached. Results for GA, PSO, and GA\&PSO methods 
are presented in each column, in that order. 
The GA algorithm (on the left column) finds it difficult to reach the
$10^{-5}$ target in functions ($f_1-f_4$), and has the worst
performance of the three on higher dimensions, but it has a better performance in $f_5$
where it reaches the $10^{-8}$ target, in this case it has the highest runtime when compared to the others. 
            ...
            % TO DO
The PSO variant on the other hand, has a better performance; reaching all targets for functions $f_1$ and $f_2$ but 
it has a higher runtime in most cases when compared against the multi-strategy method. 
PSO does not reach the $10^{-8}$ target on for the higher dimensions of functions $f_3$ and $f_4$ ; 
