...
containing solution populations from message queues. After searching for a
small number of iterations, new evolved populations are generated and
sent back to a queue. Current research suggests that when we have many 
population-processing algorithms communicating in parallel, parameters 
intensifying exploration or exploitation in each population strike a 
dynamic that balances the two, exploring and exploiting simultaneously,
maintaining an overall diversity, and improving the search. 
% Change it again - Mario 
% You should clarify this sentence. - JJ
...
compared with other asynchronous homogeneous population-based algorithms, % Asynchronous but not mixed? - JJ
% Yes - Mario
...
% We should make a stronger statement on why
...
\item Once a stateless function finishes the execution of a pre-fixed number of
  generations, % If you say "they" it looks like they work
               % asynchronously - JJ
               % They do - Mario 
  the resulting population is pushed to
...
\item Otherwise, it passes the stream of messages to a ``migration'' function,
  where populations are mixed. It generates new populations, % This
                                ...
                                % We should uncomment this comment - M
  and they are
...
%% Maybe is not important to highlight these advantages in this paper 
%% This could be form the previous paper. We can remove it - M 
...
\cite{guervos2019improving}.
                             ...
                             % Should we reference the journal paper also? - M
...
%\begin{algorithm} % I don't really think this is necessary; candidate
                  ...
                  % Removing it, this is included in the journal paper. - M
...
the BBOB testbed % which testbed?
...
as those proposed by El-Abd and Kamel in the PSO implementation for the BBOB benchmark \cite{el2009black}.
                                ...
                                % and also less parameters to tune?
...
independent of each other. % Clarify "possibly" - JJ
