...
execute the optimization strategy by reading streams of populations from message
queues. After searching for a small number of iterations, replacing them with
evolved versions of them. Moreover, current research suggests that when we have
many populations communicating in parallel, each populations' parameters are
compensated by those selected in other populations, improving the overall
performance. In this work, we propose a simple reactive migration method for the
asynchronous execution of multi-population, multi-strategy algorithms that
improve over homogeneous configurations. We evaluate this method by comparing a
...
this method offers better performance, even when compared with other
asynchronous population-based algorithms.
...
Algorithm to produce a new set of candidates considering each candidate
...
Options include the size and number of populations, the type of interaction
...
than single-population or homogeneous multi-population optimization algorithms
\cite{wu2016differential,nseef2016adaptive}. 

Heterogeneous algorithms put an
additional burden on the tuning of parameters for each population; some
...
having a high number of populations that communicate in parallel compensates for
...
search strategies have. Therefore, in this paper, we implemented an asynchronous
multi-population algorithm version, using a message queue for inter-process
communication \cite{guervos2018introducing} and a reactive migration procedure.
We compare three heterogeneous configurations using a randomized parameter
technique. We experimented with all populations using a GA or PSO search
