...
needing to keep a buffer in memory or external storage, and it follows
...
When the \texttt{population\_mixer} unit receives a list of three populations, let us say
...
\begin{algorithm} % I don't really think this is necessary; candidate
    ...
    \begin{algorithmic}[1]
        \Procedure{cxBestFromEach}{$pop_1,pop_2$}
            \State $pop_1.sort()$
            \State $pop_2.sort()$ % If you call this procedure twice
                                % for every population, wouldn't it be
                                % better to have them sorted before
                                % invoking it? - JJ
